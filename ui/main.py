...
<body class="flex flex-row overflow-hidden bg-slate-50 p-4 gap-4 h-screen">
    ...
    <aside class="w-80 bg-white/80 backdrop-blur-md border border-slate-200/80 rounded-2xl p-6 flex flex-col gap-6 shrink-0 h-full z-50 shadow-[0_10px_25px_-5px_rgba(0,0,0,0.02)]">
        ...
        <!-- Team Color Toggle -->
        <div class="flex flex-col gap-2 border-t border-slate-100 pt-4 shrink-0">
            <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest block">Team Color</span>
            <div class="grid grid-cols-2 gap-2">
                <button id="colorBtnBlue" onclick="setTeamColor('BLUE')"
                    class="py-2 text-[10px] font-black uppercase tracking-widest rounded-lg border-2 border-blue-500 bg-blue-600 text-white transition-all">
                    BLUE
                </button>
                <button id="colorBtnRed" onclick="setTeamColor('RED')"
                    class="py-2 text-[10px] font-black uppercase tracking-widest rounded-lg border-2 border-slate-200 bg-slate-50 text-slate-500 transition-all">
                    RED
                </button>
            </div>
        </div>
        ...
        <div class="bg-slate-50 border border-slate-200/60 rounded-xl p-4 flex flex-col gap-2.5 shrink-0">
            ...
            <div class="flex items-center justify-between text-[10px] font-bold text-slate-600 uppercase">
                <span>ROS State:</span>
                <span id="macroStateDisplay" class="font-mono text-slate-500">IDLE</span>
            </div>
            <div class="flex items-center justify-between text-[10px] font-bold text-slate-600 uppercase">
                <span>IMU Pitch:</span>
                <span id="imuPitchDisplay" class="font-mono text-slate-700">0.0°</span>
            </div>
            <div class="flex items-center justify-between text-[10px] font-bold text-slate-600 uppercase">
                <span>HW Error:</span>
                <span id="hwErrorDisplay" class="font-mono text-emerald-600">None</span>
            </div>
        </div>
    ...
    <script>
        ...
        async function playAnimation(routeIndex) {
            ...
            // Save selected path to Flask so bridge includes it in mission_plan on START
            const path = actions.map(a => a.to || a.target).filter(Boolean);
            fetch('/api/control/action', {
                method: 'POST', headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ action: 'SET_MEIHUA_PATH', path: path })
            }).catch(e => {});
        ...
        // ── Global command dispatcher (used by sidebar E-stop & RESET) ────
        function sendCommand(cmd) {
            ...
            if (cmd === 'DISENGAGE') {
                // cancel all task simulations
                if (typeof autoInterval !== 'undefined') clearInterval(autoInterval);
                if (typeof stepSimulationTimeout !== 'undefined') clearTimeout(stepSimulationTimeout);
                if (typeof t3AutoInterval !== 'undefined') clearInterval(t3AutoInterval);
                if (typeof t3ConflictingTimeout !== 'undefined' && t3ConflictingTimeout) clearTimeout(t3ConflictingTimeout);
                if (typeof isTask3Running !== 'undefined') isTask3Running = false;

                const task1Log = document.getElementById('task1Log');
                if (task1Log) task1Log.innerHTML += '<br><span class="text-rose-600 font-bold">> [GLOBAL] EMERGENCY STOP sent to ROS2.</span>';

                const startBtn = document.getElementById('btnTask1Start');
                if (startBtn) {
                    startBtn.innerHTML = '<span class="relative z-10">START AUTO RUN</span><div class="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>';
                    startBtn.className = "flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-black text-xs uppercase tracking-widest py-3.5 rounded-xl shadow-md transition-all active:scale-95 border-2 border-indigo-500 relative overflow-hidden group";
                }
            }
        }
        ...
        function setTeamColor(color) {
            fetch('/api/control/action', {
                method: 'POST', headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ action: 'SET_TEAM_COLOR', color: color })
            }).catch(e => {});

            const activeClass = "py-2 text-[10px] font-black uppercase tracking-widest rounded-lg border-2 transition-all";
            document.getElementById('colorBtnBlue').className = activeClass +
                (color === 'BLUE' ? ' border-blue-500 bg-blue-600 text-white' : ' border-slate-200 bg-slate-50 text-slate-500');
            document.getElementById('colorBtnRed').className = activeClass +
                (color === 'RED' ? ' border-rose-500 bg-rose-600 text-white' : ' border-slate-200 bg-slate-50 text-slate-500');
        }

        // ── Macro-state live update (poll /api/status every 1s) ──────────
        setInterval(() => {
            fetch('/api/status').then(r => r.json()).then(d => {
                const el = document.getElementById('macroStateDisplay');
                if (!el) return;
                const stateColors = {
                    IDLE: 'text-slate-500',
                    WEAPON_CLUB: 'text-indigo-600',
                    MEIHUA_FOREST_EXECUTION: 'text-emerald-600',
                    MARTIAL_ART_PLACEMENT: 'text-amber-600',
                };
                el.className = 'font-mono ' + (stateColors[d.macro_state] || 'text-slate-500');
                el.innerText = d.macro_state || 'IDLE';

                const pitch = document.getElementById('imuPitchDisplay');
                if (pitch) pitch.innerText = (d.imu_pitch || 0).toFixed(1) + '°';

                const hw = document.getElementById('hwErrorDisplay');
                if (hw) {
                    hw.innerText = d.hardware_error || 'None';
                    hw.className = 'font-mono ' + (d.hardware_error && d.hardware_error !== 'None' ? 'text-rose-600' : 'text-emerald-600');
                }
            }).catch(e => {});
        }, 1000);
        ...
        function startTask1() {
            ...
            if (btn.innerText.includes('START')) {
                ...
                // Notify backend of command (team_color already stored via setTeamColor)
                fetch('/api/control/action', {
                    method: 'POST', headers: {'Content-Type': 'application/json'},
        ...
        function setTicTacToeSlot(num) {
            ...
            // Persist to Flask so bridge picks it up on START
            fetch('/api/control/action', {
                method: 'POST', headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ action: 'SET_TTT_SLOT', slot: num })
            }).catch(e => {});
        }
...
robot_state = {
    ...
    # operator config — sent to bridge on START
    "team_color": "BLUE",           # "RED" | "BLUE"
    "meihua_forest_path": [],       # list of block IDs in order
    "ttt_target": "middle",         # "left" | "middle" | "right"
    "yolo_conf": 0.75,
    # populated by ROS2 bridge node
    "macro_state": "IDLE",
    "imu_pitch": 0.0,
    "hardware_error": "None",
}
